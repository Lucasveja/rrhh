...
if($_POST['boton']=="Guardar")
{
	  ...
	  foreach ($ruslt->rows as $read) 
	  {
		...
		{
		   $sqlGlobal="insert into desempenio(periodo,idempresa,idevaluador,idevaluado,porcentaje,resultado) values('{$_POST['periodo']}','{$idEmpresa}','{$idEvaluador}','{$_POST['evaluado']}','{$_POST['porc']}','{$_POST['optradio']}')";
		   
		   if(!mysql_query($sqlGlobal))
		   {//ya estaba en la tabla
		      $sqlGlobalUp="update desempenio set porcentaje='{$_POST['porc']}',resultado='{$_POST['optradio']}' where periodo='{$_POST['periodo']}' and idempresa='{$idEmpresa}' and idevaluador='{$idEvaluador}' and idevaluado='{$_POST['evaluado']}'";
