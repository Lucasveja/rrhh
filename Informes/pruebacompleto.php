...
<html lang="en">
<head>
  <title>Developsys/Informes Evaluado Completo</title>
...
//Validar que el usuario este logueado y exista un UID
 ...
 //$content ="<page backtop='80mm'><page_header></page_header></page>";
  ...
  //$p= explode("/",$peri);
  //$peri=$p[0].$p[1];
  $desde=($_GET['mtpl']*30)-30;
  if($desde<>0)
  {
    $desde=$desde+1;
  }
  
  $sqlTodos="select * from movimientos WHERE IdEmpresa='{$_GET[empresa]}' and IdEvaluador='{$_GET[evaluador]}' and Periodo='{$peri}' group by IdEvaluado  limit 31,2";

  $resTodos=mysql_query($sqlTodos);
  $roww=mysql_fetch_array($resTodos);
   $h=0;
