...
if($_POST['boton']=="Guardar")
{
	  ...
	  foreach ($ruslt->rows as $read) 
	  {
		 ...
		 if($_POST[$nivel]<>'')
		 {
		  ...
		  $sqlIn="insert into movimientos(periodo,idempresa,idevaluador,idevaluado,fechacarga,idcompetencia,competencia,nivelalcanzado,idnivel) value('{$_POST['periodo']}','{$idEmpresa}','{$idEvaluador}','{$_POST['evaluado']}',curdate(),'{$read['id']}','{$read['competencia']}','{$_POST[$nivel]}','{$_POST[$idniv]}')";
...
a.idevaluador='{$idEvaluador}' and a.idempresa='{$idEmpresa}' and a.periodo='2019'";
			   $ruslt=query($sqlOs);
			  foreach ($ruslt->rows as $read) 
    	     {
...
<?
   if($_POST['boton']=="Buscar")
   {
    ...
    //$periHay=$per[0].$per[1];
	$sqlDes="Select * from desempenio where periodo='{$_POST['periodo']}' and idevaluado='{$_POST['evaluado']}' and  idempresa='{$idEmpresa}'";
	...
	$sqlH="Select  titulo,nivelalcanzado from datos.movimientos m inner join datos.competencias c  where m.periodo='{$_POST['periodo']}' and m.idevaluado='{$_POST['evaluado']}' and  m.idempresa='{$idEmpresa}' and m.competencia=c.competencia";
     $consultaH= mysql_query($sqlH);
?>
 <ul class="list-group">
<?
     while($rowH= mysql_fetch_array($consultaH))
    {
     ?>
     
  
 		 <li class="list-group-item"><? echo $rowH['titulo']?><span class="badge badge-primary badge-pill"><? echo $rowH['nivelalcanzado']?></span></li>
...
<h3 align="center">Desempeño Global </h3>
	...
	<div class="form-group  has-feedback ">         
	 ...
	 <label class="col-sm-5 control-label" >Fortaleza</label>	
		    <div class="col-sm-7">
			 <textarea name="fortaleza" cols="50" rows="5"><? echo $rowDes['fortalezas']?></textarea>
			</div> 	 	  
		</div>		
	 ...
	 <label class="col-sm-5 control-label" >Debilidades</label>	
		    <div class="col-sm-7">
			 <textarea name="debilidades" cols="50" rows="5"><? echo $rowDes['debilidades']?></textarea>
			</div> 	 	  
		</div>		
	 ...
	 <label class="col-sm-5 control-label" >Compromiso</label>	
		    <div class="col-sm-7">
			 <textarea name="compromiso" cols="50" rows="5"><? echo $rowDes['compromiso']?></textarea>
			</div> 	 	  
		</div>		
	 ...
	 <label class="col-sm-5 control-label" >Capacitacion</label>	
		    <div class="col-sm-7">
			 <textarea name="capacitacion" cols="50" rows="5"><? echo $rowDes['capacitacion']?></textarea>
			</div> 	 	  
		</div>				
...
<input name="marcado" id="marcado" type="hidden" value="<? echo $marca;?>">
