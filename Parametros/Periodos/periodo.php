...
function editar_nombre($id, $nuevo){
	...
	$sql="UPDATE periodos SET Periodo='{$nuevo}' Where Id='{$id}'";
	$res=mysql_query($sql);

	$js="$('#tabla_periodo').load('tabla.php');$('#Modal_editar').modal('hide');";
	$objResp->script($js);
...
function eliminar_periodo($id){
	...
	$res=mysql_query($sql);
	$js="$('#tabla_periodo').load('tabla.php');";
	$objResp->script($js);
...
function guardar_periodo($nom){

	$objResp=new xajaxResponse();
	$conn = new conexionBD (); //Genera una nueva coneccion

	//$objResp->alert("XAJAX ".$nom);
	...
	$sqll="Select * from periodos Where Periodo='{$nom}'";
	//$objResp->alert($sql);
	$resl=mysql_query($sqll);
	$n=mysql_num_rows($resl);
	...
	if($flag==1){
		$JS="alerta('CUIDADO', 'El <strong>Periodo</strong> no puede ser Vacio.', 'info', 'warning-sign');";
		$objResp->script($JS);

	}

	if($flag1==1){
		$JS1="alerta('CUIDADO', 'El <strong>Periodo</strong> ya existe.', 'info', 'warning-sign');";
		$objResp->script($JS1);
	...
	if($flag==0 && $flag1==0){
		...
		$sqlSet="Update periodos SET PeriodoActivo=0";
    	mysql_query($sqlSet);

    	$sql="INSERT INTO periodos (Periodo,PeriodoActivo) VALUES ('{$nom}','1')";
    	//echo $sql;
    	$res=mysql_query($sql);
			
			$objResp->assign('nombre','value','');
			$JS2="$('#tabla_periodo').load('tabla.php');";
			$objResp->script($JS2);
...
$xajax->registerFunction("guardar_periodo");
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <script>
	...
	function editar(id, nom){
		...
		document.getElementById('Periodo_edit').value=nom;
	}
	...
	function eliminar(id, nom){
		...
		bootbox.confirm({
    title: "Eliminar ",
    message: "Eliminar el Periodo <strong>"+nom+"</strong> ?",
    closeButton: false,
    buttons: {
        cancel: {
            label: 'Cancelar'
	...
	function editar_nombre(id, nuevo){
		...
		if(nuevo==""){
			...
			alerta("CUIDADO", "El <strong>Periodo Nuevo</strong> No puede estar Vacio.", "info", "warning-sign");
			return false;
...
<body>
    ...
    // 	$sql="INSERT INTO periodos (Periodo,PeriodoActivo) VALUES ('$_POST[nombre]',1)";
    // 	//echo $sql;
    // 	$res=mysql_query($sql);
		...
		<form class="form-horizontal" role="form"  id="formu" name="formu" method="post">
			...
			<div class="row">
				<div class="form-group">
					<div class="col-sm-3">
						<input type="text" class="form-control" name="nombre" id="nombre" placeholder="Ingrese Periodo Activo">
					</div>
					<input type="button" class="btn btn-primary" name="Guardar" id="Guardar" value="Guardar" onclick="guardar(nombre.value);">
				</div>
			...
			<div id="tabla_periodo" style="display:block">
...
<div class="modal fade" id="Modal_editar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
			   ...
			   <div id="Editar" name="Editar" > 
		           <div class="form-group  has-feedback">
		              ...
		              <div class="col-sm-3">
		                ...
		                <input name="Periodo_edit" id="Periodo_edit" type="text" class="form-control"  value="" placeholder="Ingrese Periodo">
      ...
      <input name="oculto" id="oculto" type="hidden" class="form-control col-sm-2"  value="<? echo $_POST['oculto']?>" placeholder="Ingrese Periodo">
        <button type="button" class="btn btn-default" data-dismiss="modal" >Cancelar</button>
        <button type="button" class="btn btn-primary" value="Listo" id="Listo" name="Listo"  onclick="return editar_nombre(oculto.value,Periodo_edit.value)">Listo</button>
      </div>
    </div>
  </div>
</div>
