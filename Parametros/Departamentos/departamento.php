...
function editar_depto($id, $nuevo, $emp){
	...
	$sql="UPDATE departamentos SET Depto='{$nuevo}', IdEmpresa='{$emp}' Where Id='{$id}'";
	$res=mysql_query($sql);

	$js="$('#tabla_depto').load('tabla.php');$('#Modal_editar').modal('hide');";
	$objResp->script($js);
...
$xajax->registerFunction("editar_depto");
$xajax->registerFunction("eliminar_depto");
...
<title>RRHH/Departamentos</title>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <script>
	...
	function editar(id, nom, emp){
		...
		document.getElementById('Depto_edit').value=nom;
		//alert(emp);
	...
	function eliminar(id, nom){
		...
		bootbox.confirm({
	    title: "Eliminar ",
	    message: "Eliminar Departamento <strong>"+nom+"</strong> ?",
	    closeButton: false,
	    buttons: {
	        cancel: {
	            label: 'Cancelar'
	...
	function editar_nombre(id, nuevo, emp){
		...
		if(nuevo=="" || emp=="-1" ){
			...
			xajax_editar_depto(id,nuevo,emp);
		}
...
<body>
    ...
    if($_POST[Guardar]){
    	...
    	$sql="INSERT INTO departamentos (depto,IdEmpresa) VALUES ('$_POST[nombre]','$_POST[Empresa]')";
    	//echo $sql;
    	$res=mysql_query($sql);
		...
		<form class="form-horizontal" role="form"  id="formu" name="formu" method="post">
			...
			<div class="row">
				<div class="form-group">
					<div class="col-sm-3">
						<input type="text" class="form-control" name="nombre" id="nombre" placeholder="Nombre de Departamento">
						</div>
						<div class="col-sm-3">
			...
			<div id="tabla_depto" style="display:block">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
			   ...
			   <div id="Editar" name="Editar" > 
		           <div class="form-group  has-feedback">
		              ...
		              <div class="col-sm-3">
		                ...
		                <input name="Depto_edit" id="Depto_edit" type="text" class="form-control"  value="" placeholder="Nombre de Area">
      ...
      <input name="oculto" id="oculto" type="hidden" class="form-control col-sm-2"  value="<? echo $_POST['oculto']?>" >
        ...
        <button type="button" class="btn btn-primary" value="Listo" id="Listo" name="Listo"  onclick="return editar_nombre(oculto.value,Depto_edit.value, Empresa_edit.value)">Listo</button>
      </div>
    </div>
  </div>
</div>
